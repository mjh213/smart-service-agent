...
- **Agent 工具调用闭环**：将商品查询、商品卡片发送、客服知识检索、转人工等业务动作封装为工具，由 Agent 根据用户意图选择调用。
- **RAG 可解释演示**：本地测试页展示知识命中来源、工具调用、LLM 调用次数、耗时、Token 粗估和兜底状态，方便排查 AI 回复质量。
- **工程化消息处理**：采用消息队列、处理器链、连接状态管理和自动重连机制，避免把 AI 回复逻辑写成单次脚本调用。
- **桌面端可落地形态**：使用 PyQt6 构建账号管理、知识库、自动回复、本地测试、日志和配置界面，可打包为 Windows 桌面应用。

## 功能特性

- **渠道接入能力**：当前已接入电商平台实时消息通道，可持续扩展更多渠道
- **AI 智能回复**：基于内置 Agent 编排框架，多轮工具调用 + 会话上下文管理
...
启动后可在左侧导航栏进入 `本地测试` 页面，直接输入问题演示 Agent 的问答与知识检索能力；页面左侧显示 Agent 对话，右侧显示本次问题命中的 RAG 检索结果和回答分析，无需依赖真实平台来发送测试消息。
...
仓库中提供了 `config.example.json`，复制为 `config.json` 后填入自己的模型配置即可。真实的 `config.json`、数据库、日志、浏览器缓存和用户数据已加入 `.gitignore`，不建议上传到公开仓库。

## 面试演示建议

推荐演示路径：

1. 运行 `python scripts/inject_hardcore_data.py` 注入演示数据。
2. 启动应用并进入 `本地测试` 页面。
3. 输入商品咨询、售后政策、转人工等问题。
4. 重点讲解右侧回答分析面板中的 RAG 命中、工具调用、耗时、Token 粗估和兜底状态。

可用于面试表达的核心点：

- 不是简单调用大模型，而是把 LLM、RAG、工具调用、业务状态和桌面端 UI 组合成完整 AI 应用。
- 本地测试页具备可解释性，能展示 AI 为什么这样回答，以及系统如何评估一次回复。
- 知识库没有命中时会进入兜底策略，避免模型胡编业务规则。
