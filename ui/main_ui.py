...
class MainWindow(FluentWindow):
    def __init__(self):
        super().__init__()
        self.setProperty("themeWindow", True)
        t = time.perf_counter()
        self.setWindowTitle('通用电商智能 Agent 控制台')
    ...
    def lazy_load_views(self):
        ...
        t = time.perf_counter()
        from ui.local_test_ui import LocalTestUI
        self.logger.info(f"  import LocalTestUI: {time.perf_counter()-t:.2f}s")
        t = time.perf_counter()
        ...
        t = time.perf_counter()
        self.local_test_view = LocalTestUI(self)
        self.logger.info(f"  LocalTestUI: {time.perf_counter()-t:.2f}s")
        t = time.perf_counter()
    ...
    def initNavigation(self):
        ...
        self.navigationInterface.setProperty("themeNav", True)
        self.addSubInterface(self.monitor_view, FIF.CHAT, '自动回复')
        self.addSubInterface(self.local_test_view, FIF.CHAT, '本地测试')
    ...
    def initWindow(self):
        ...
        if hasattr(self, "titleBar") and self.titleBar:
            self.titleBar.setProperty("themeNav", True)
