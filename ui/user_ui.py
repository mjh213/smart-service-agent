...
class LoginThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            if username.startswith("mock_") or username.startswith("test_"):
                import time
                time.sleep(0.8)  # 模拟一小会儿网络请求的延迟感，让演示更逼真

                # 伪造一个完美的登录成功数据包，直接喂给后面的入库逻辑
                mock_result = {
                    "channel_name": "pinduoduo",  # 虚拟渠道名称
                    "shop_id": f"shop_{username}",  # 动态生成虚拟店铺ID
                    "shop_name": f"【测试节点】{username}",  # 界面上显示的店铺名
                    "user_id": f"uid_{username}",
                    "username": username,
                    "password": self.account_data.get("password", ""),
                    "shop_logo": "",  # 为空则触发无Logo的默认样式
                    "cookies": '{"is_mock": true}'
                }
                self.login_finished.emit(mock_result)
                return  # 提前终止，坚决不走下方真实的 login_pdd 网络请求
            # ==========================================================
...
class AccountCard(CardWidget):
    ...
    def __init__(self, account_data: dict, parent=None):
        ...
        self.platform = get_channel_display_name(account_data.get("channel_name", ""))
    ...
    def createLogoWidget(self):
        ...
        self.logo_label.setProperty("themeLogo", True)
    ...
    def createInfoWidget(self):
        ...
        shop_name_label.setStyleSheet("color: #4f4336;")
    ...
    def createInfoRow(self, label_text: str, value_text: str):
        ...
        label.setStyleSheet("color: #7b6b58; font-weight: 500;")
        label.setFixedWidth(60)
        
        # 值
        value = BodyLabel(value_text)
        value.setStyleSheet("color: #5b4d3e;")
...
class EditAccountDialog(QDialog):
    ...
    def setupUI(self):
        ...
        # 渠道信息（只读）
        self.channel_label = QLabel(get_channel_display_name(self.account_data["channel_name"]))
...
class AddAccountDialog(QDialog):
    ...
    def setupUI(self):
        ...
        self.username_edit.setPlaceholderText("请输入电商平台授权码")
        self.username_edit.setFixedSize(300, 32)
        self.password_edit = LineEdit(self)
        self.password_edit.setEchoMode(LineEdit.EchoMode.Password)
        self.password_edit.setPlaceholderText("请输入App Secret")
        self.password_edit.setFixedSize(300, 32)
        layout.addRow("App Key:", self.username_edit)
        layout.addRow("App Secret:", self.password_edit)
